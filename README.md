...
13. **Brick-and-Mortar Store** — the major mid-game upgrade ($8,000). Opens a physical shop: **in-person walk-in customers** start arriving (on top of your online orders), and you can accept **Cash**. Card *shows* stay open from the start regardless — the store is specifically about home/local foot traffic.
    ...
    - **Walk-ins ask for things** — a share of customers come in *hunting a specific item* (a sealed product or a single). Have it **on the shelf** → an instant happy sale; only in **the back** (your collection/inventory) → grab it from the back; **don't own it at all** → they leave disappointed (a demand signal + small rep ding). Keeping the shelf stocked to match local demand is the shopkeeper's game.
...
15. **Prices** (a sub-tab under **Collection**) — a reference price guide: pick any set and browse every card with its market value; search by name, sort by number / value / rarity. Shows the set's total value and chase card. Multi-day shows also re-roll their vendor floor each day — different booths and stock day to day.
