...
2. **Rip** — click the pack to tear it open; cards reveal one by one, hits get a confetti burst and are held to the end. Cards have a subtle pointer-tracked 3D tilt on hover; **rarity is shown by the card's border colour** (common → gray, up through gold/pink for the chases) so the artwork is never covered. Hits glow in their rarity colour as they land. Tune the experience in **Settings → Rip speed** (Slow / Normal / Fast / Turbo) and **Auto-open next pack** (in one-at-a-time mode, auto-rips the next pack a few seconds after each finishes).
   - **Special foils** (Prismatic Evolutions & Black Bolt): a card in the reverse slot can come as a **⦿ Poké Ball Foil** (~1 in 3, 1.6× value) or, in Prismatic, a chase **◉ Master Ball Foil** (~1 in 19, **6× value**) — each with its own badge and border colour, and a **callout banner during the rip** so you don't miss it.
