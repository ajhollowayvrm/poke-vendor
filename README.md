...
### The dealer's craft
16. **Card condition** — every raw card has a condition: **NM / LP / MP / DMG**. Condition both **scales the card's value** (NM 100% → LP 72% → MP 45% → DMG 20%) and **caps the grade** it can earn (a Damaged card can never grade above PSA 4, MP ≤ 6, LP ≤ 8). Sealed pulls come in mostly NM; cards bought off a show floor or bargain bin run rougher. A condition chip shows on any non-NM raw card, and the grader warns you before you waste a fee on a card that can't 10.
