...
def call(url, key):
    r = subprocess.run(["curl", "-s", "-m", "300", "-D", "-", "-H", f"Authorization: Bearer {key}", url],
    ...
    headers = {k.strip().lower(): v.strip() for k, _, v in (l.partition(":") for l in head.split("\r\n")[1:])}
...
def main():
    ...
    for gid in set_ids:
        ...
        status, headers, body = call(url, key)
        if " 200 " not in status + " ":
            print(f"set {gid}: {status} {body[:200]}", flush=True)
            if " 429 " in status + " " or " 403 " in status + " ":
                break
            time.sleep(5)
            continue
        try:
            ...
            print(f"set {gid}: response is not JSON", flush=True)
            continue
        ...
        print(f"set {gid}: {len(cards)} cards, {used} credits, daily credits left {rem}", flush=True)
        time.sleep(1.5)
