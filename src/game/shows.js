...
const INPERSON_METHODS = ['cash', 'venmo', 'card', 'paypal', 'tap'] // 📲 tap is counter-only — no phone-tapping a web checkout
...
export function makeGiftBuyer(s, accepted = null, notoriety = 0) {
  ...
  // 🎁 Gift Wrap Counter: a real wrapping station earns a fatter premium — and lets you fetch
  // a giftable sealed from the BACK when the floor has nothing in budget ("give me a minute,
  // I've got just the thing in the storeroom"). 🔒 kept and held stock stays invisible.
  const wrapCounter = !!s.upgrades?.giftWrap
  const wrapPrem = GIFT_WRAP_PREMIUM + (wrapCounter ? 0.08 : 0)
  const giftable = it => !it.locked && !it._heldFor && (it.loc === 'floor' || wrapCounter)
  const sealedPick = (s.sealedInventory || [])
    .filter(giftable)
    .map(it => ({ it, price: round2(sealedValue(it) * (1 + SEALED_SHOP_MARKUP + wrapPrem)) }))
    .filter(x => x.price <= budget)
    // in-budget floor stock first (the back is the fallback), then priciest — best gift wins
    .sort((a, b) => (b.it.loc === 'floor') - (a.it.loc === 'floor') || b.price - a.price)[0]
  const cardPick = sealedPick ? null : (s.collection || [])
    .filter(c => c.loc === 'floor' && !c.locked && !c._heldFor)
    .map(c => ({ c, price: round2(cardValue(c) * (1 + STORE_SALE_PREMIUM + wrapPrem)) }))
...
export function makeConsignRequest(notoriety, opts = {}) {
  ...
  return {
    ...
    card, ask,
    // your cut when it sells (15–25%; the 🗄️ locking case justifies a few points more)
    commissionPct: round2(0.15 + Math.random() * 0.10 + (opts.casePlus ? 0.03 : 0)),
...
const PAY_LABELS = { venmo:'Venmo', cash:'cash', paypal:'PayPal', card:'card', tap:'tap' }
